...
> ⚠️ **Escopo exclusivo: back-end.** Você implementa rotas, controllers, services, repositories, models, migrations, middleware, validações, jobs e integrações. Você não implementa frontend, componentes visuais, telas ou estilos.
...
- `## Story alvo` — bloco da Story copiado do backlog.md pelo Orchestrator (critérios de aceite, tipo, módulos afetados)
- `## API Spec — endpoints desta Story` — seções de endpoint do api-spec-epic-XX.md relevantes para esta Story, extraídas pelo Orchestrator (obrigatório quando existir; não implemente sem elas)
- Código existente relevante — entenda os contratos (interfaces, tipos, schemas, rotas, serviços) que a Story vai tocar
...
Verifique o campo **Tipo** e **Módulos afetados** da Story:

**Se Tipo = 🆕 Nova feature e Módulos afetados = "nenhum — criação nova":**
- Pule para o Passo 1

**Se Tipo = 🔀 Melhoria, 🔧 Refactoring ou 🐛 Bugfix:**
- Para cada arquivo listado em "Módulos afetados", leia o código atual
- Documente mentalmente:
  - Quem consome este serviço/rota? (quais módulos dependem dele)
  - Qual é o contrato atual? (request, response, side effects)
...
- Liste os arquivos que serão criados ou modificados (confirme com os "Módulos afetados" da Story)

### Passo 1B — Verificar dependências de infraestrutura (obrigatório)

Antes de planejar, identifique todas as dependências de infraestrutura que a Story exige:

1. Liste cada serviço externo que a Story precisa (banco de dados, filas, cache, serviços terceiros, etc.)
2. Para cada um, verifique se a configuração **já existe** no projeto (variáveis de ambiente, conexões, clients configurados)
3. Se a dependência **não for localizada**:
   - **Não bloqueie a implementação** — implemente com mock/stub temporário
   - **Registre a pendência** no relatório `{WORK_DIR}/us-XX-infra-pendencias.md` usando o template da `development/SKILL.md`
   - Adicione um comentário no código: `// TODO(US-XX): configurar quando infraestrutura disponível`
   - Sinalize ao **Orchestrator-Dev** que há pendências de infraestrutura

> ⚠️ Se **todas** as dependências críticas da Story estiverem ausentes, pare e consulte o Orchestrator-Dev antes de prosseguir.
...
- [ ] Nenhum consumidor do serviço/módulo alterado foi quebrado (revise quem importa os arquivos modificados)
- [ ] Nenhum contrato de API público foi removido ou alterado sem documentar a migração
...
- **Pendências de infraestrutura:** sempre que uma dependência necessária não for localizada, gere o relatório `us-XX-infra-pendencias.md` — nunca ignore a ausência silenciosamente.
