...
description: Context mounting protocols, Epic integration, feedback loops, and tech-debt management. Loaded by orchestrator-core only when activating sub-agents or running advanced protocols.
user-invocable: false
---

# Orchestrator-Dev — Protocolos
...
| UI Agent | `.claude/agents/dev/ui.md` | `.claude/skills/ui/SKILL.md` |
...
**Convenção de UI specs:** `{WORK_DIR}/ui-epic-XX.md` (ex: EPIC-01 → `ui-epic-01.md`).
...
[bloco completo da US-XX: título, narrativa, critérios de aceite, tipo, estimativa, dependências, componentes afetados]

## UI Spec — telas desta Story (extraído de ui-epic-XX.md)
[apenas as seções "### Tela: [Nome]" da Story alvo]
```
...
1. Acione **QA & Docs** com:
   ...
   - `{WORK_DIR}/ui-[epic].md` — coerência visual
...
## Protocolo de contratos de API

**Ownership:** o Developer registra contratos consumidos em `docs/api-contracts.md` durante a implementação. O QA valida se novos contratos foram documentados (ver `qa-docs/SKILL.md` — "Quando documentar").

**Atualização:** quando uma Story consome um endpoint já documentado mas com contrato diferente do registrado, o Developer deve atualizar `docs/api-contracts.md` na mesma branch da Story e registrar a mudança no `us-XX-entrega.md`.
